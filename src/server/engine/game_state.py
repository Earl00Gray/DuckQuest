...
from server.engine.game_state_item import History, Context, Character, Inventory, HistoryItem
from server.common.results.I_Result import I_Result, ResultType
from server.engine.action_result import LootResult, ExpereanceResult, LootResultType
...
class GameState:
    ...
    def __applyExpereance(self, result_item: ExpereanceResult) -> None:
        ...
        if result_item.var.startswith(f"{Character.prefix}."):
            self.__character.applyOperation(
                result_item.var, result_item.value, result_item.operation)
        elif result_item.var.startswith(f"{Context.prefix}."):
            self.__context.applyOperation(
                result_item.var, result_item.value, result_item.operation)
